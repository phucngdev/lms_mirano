...
const VerifyOtpForgotPassword = () => {
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const verifyData: VerifyCodeInputDto = {
        email: email,
        action: VerifyCodeInputDto.action.RESET_PASSWORD,
        verifyCode: otp,
      };

      const response = await verifyOtpService(verifyData);

      if (response.data.statusCode === 200 || response.data.statusCode === 201) {
        message.success('Xác thực OTP thành công!');
        navigate('/auth/reset-password', {
          state: { email, verifyCode: otp },
        });
      } else {
  ...
  const handleResendOtp = async () => {
    ...
    message.info('Vui lòng quay lại trang quên mật khẩu để gửi lại mã OTP');
